...
window.FarmGod.Main = (function (Library, Translation) {
      ...
      if (game_data.screen == 'am_farm') {
        $.when(buildOptions()).then((html) => {
          ...
          $('.optionButton').off('click').on('click', () => {
            ...
            let optionSortOrder = $('.optionSortOrder').val();
            ...
            getData(optionGroup, optionNewbarbs, optionLosses).then((data) => {
              ...
              let plan = createPlanning(optionDistance, optionTime, optionMaxloot, optionSortOrder, data);
      ...
      return `<style>#popup_box_FarmGod{text-align:center;width:550px;}</style>
                ...
                <div style="width:90%;margin:auto;background: url(\'graphic/index/main_bg.jpg\') 100% 0% #E3D5B3;border: 1px solid #7D510F;border-collapse: separate !important;border-spacing: 0px !important;"><table class="vis" style="width:100%;text-align:left;font-size:11px;">
                  ...
                  <tr><td>Sort order:</td><td>
                    <select class="optionSortOrder">
                      <option value="asc" ${options.optionSortOrder === 'asc' ? 'selected' : ''}>From closest</option>
                      <option value="desc" ${options.optionSortOrder === 'desc' ? 'selected' : ''}>From farthest</option>
                    </select>
                  </td></tr>
  ...
  const createPlanning = function (optionDistance, optionTime, optionMaxloot, sortOrder, data) {
    let plan = { counter: 0, farms: [] };
    let serverTime = Math.round(lib.getCurrentServerTime() / 1000);

    let possibleCombos = [];

    for (let origin in data.villages) {
      for (let target in data.farms.farms) {
        let distance = lib.getDistance(origin, target);
    ...
    possibleCombos.sort((a, b) => b.distance - a.distance);

    for (let combo of possibleCombos) {
      let farmIndex = data.farms.farms[combo.target];
      ...
      let unitsLeft = lib.subtractArrays(data.villages[combo.origin].units, template.units);

      let arrival = Math.round(serverTime + ((combo.distance * template.speed) * 60) + Math.round(plan.counter / 5));
      let maxTimeDiff = Math.round(optionTime * 60);
      let timeDiff = true;

      if (data.commands.hasOwnProperty(combo.target)) {
        if (!farmIndex.hasOwnProperty('color') && data.commands[combo.target].length > 0) timeDiff = false;
        data.commands[combo.target].forEach((timestamp) => {
          if (Math.abs(timestamp - arrival) < maxTimeDiff) timeDiff = false;
        });
      } else {
        data.commands[combo.target] = [];
      }
      ...
      if (unitsLeft && timeDiff) {
        ...
        plan.farms.push({
          'origin': {
            'coord': combo.origin,
            'name': data.villages[combo.origin].name,
            'id': data.villages[combo.origin].id
          },
          'target': {
            'coord': combo.target,
            'id': farmIndex.id
          },
          'fields': combo.distance,
          'template': {
            'name': template_name,
            'id': template.id
          }
        ...
        data.villages[combo.origin].units = unitsLeft;
        data.commands[combo.target].push(arrival);
      }
    }

    plan.farms.sort((a, b) => {
      return sortOrder === 'asc' ? a.fields - b.fields : b.fields - a.fields;
    });
