...
window.FarmGod.Main = (function (Library, Translation) {
  ...
  const createPlanning = function (optionDistance, optionTime, optionMaxloot, data) {
    ...
    possibleCombos.sort((a, b) => b.distance - a.distance); // Sortowanie od najdalszych
    ...
    for (let combo of possibleCombos) {
      ...
      if (unitsLeft && timeDiff) {
        plan.counter++;
        if (!plan.farms.hasOwnProperty(combo.origin)) plan.farms[combo.origin] = [];

        plan.farms[combo.origin].push({
          'origin': {
            'coord': combo.origin,
