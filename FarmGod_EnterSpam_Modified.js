...
window.FarmGod.Main = (function (Library, Translation) {
  ...
  const bindEventHandlers = function () {
    ...
    // --- SPAM ENTER (TOGGLE ON/OFF + GUI BUTTON) ---
    let enterSpamActive = false;
    let enterSpamInterval = null;

    const startEnterSpam = () => {
        if (enterSpamActive) return;

        enterSpamActive = true;
        $('#FarmGodEnterSpamButton').text('Enter Spam: ON');
        UI.InfoMessage('Enter spam: ON');

        enterSpamInterval = setInterval(() => {
            const $btn = $('.farmGod_icon:visible').first();
            if ($btn.length) {
                $btn.trigger('click');
            } else {
                stopEnterSpam('Brak dostępnych celów – spam zatrzymany.');
            }
        }, Math.floor(Math.random() * (260 - 201 + 1)) + 201);
    };

    const stopEnterSpam = (msg = 'Enter spam: OFF') => {
        enterSpamActive = false;
        clearInterval(enterSpamInterval);
        $('#FarmGodEnterSpamButton').text('Enter Spam: OFF');
        UI.InfoMessage(msg);
    };

    const toggleEnterSpam = () => {
        if (enterSpamActive) {
            stopEnterSpam();
        } else {
            startEnterSpam();
    ...
    $(document).off('keydown.farmgod').on('keydown.farmgod', (event) => {
        if ((event.keyCode || event.which) === 13 && !event.repeat) {
            toggleEnterSpam();
        }
    });

    // Dodaj przycisk Enter Spam do UI
    if (!$('#FarmGodEnterSpamButton').length && $('#FarmGodProgessbar').length) {
        const btn = $(`
            <button id="FarmGodEnterSpamButton" class="btn" style="margin:10px 0 5px 0;">
                Enter Spam: OFF
            </button>
        `);

        btn.on('click', toggleEnterSpam);
        $('#FarmGodProgessbar').parent().before(
            $('<tr><td colspan="4" style="text-align:center;"></td></tr>').find('td').append(btn).end()
        );
    }
